    ...
    p.getDirection = function (new_x, new_y) {
        switch (this.swipeType) {
            ...
            case'vertical':
                ...
                break;
            case'all':
                if (Math.abs(new_x - this.start_x) > Math.abs(new_y - this.start_y)) return new_x <= this.start_x ? 'left' : 'right'; else
                    return new_y <= this.start_y ? 'up' : 'down';
                break;
        }
    };
...
;(function () {
    ...
    p.init = function (w, h) {
        ...
        switch (this.type) {
            ...
            case'center':
                this.$container.css('background-image', 'url(' + this.$img.attr('src') + ')');
                this.$container.css({backgroundPosition: 'center center', backgroundRepeat: 'no-repeat'});
                this.$img.remove();
                break;
            case'stretch':
...
;(function ($) {
    window.MSLayerElement = function () {
        this.start_anim = {name: 'fade', duration: 1000, ease: 'linear', delay: 0};
        this.end_anim = {duration: 1000, ease: 'linear'};
        this.type = 'text';
        this.resizable = true;
        this.minWidth = -1;
        this.isVisible = true;
    ...
    p.locate = function () {
        ...
        var width = parseFloat(this.layersCont.css('width')), height = parseFloat(this.layersCont.css('height')),
            factor, isPosition;
...
;(function ($) {
    ...
    p._alignPolicy = function () {
        var h = this.tt.outerHeight(false), w = Math.max(this.tt.outerWidth(false), parseInt(this.tt.css('max-width'))),
...
;(function (window, document, $) {
    ...
    p.select = function () {
        ...
        this.selected = true;
        this.pselected = false;
        this.$element.addClass('ms-sl-selected animated');
        if (this.hasLayers) {
            if (this.slider.options.autoHeight) {
    ...
    p.unselect = function () {
        ...
        this.selected = false;
        this.$element.removeClass('ms-sl-selected animated');
...
;(function ($) {
    ...
    p.__curentSlideInHash = function () {
        var hash = window.location.hash, dl = this.so.deepLink, dlt = this.so.deepLinkType,
            eq = dlt === 'path' ? '\/' : '=', regTest = new RegExp(dl + eq + '[0-9]+', 'g');
...
;(function ($) {
    ...
    p.__updateSlidesZindex = function () {
        ...
        if (this.loop) {
            ...
            var beforeNum = this.currentSlide.index - this.viewSlidesList[0].index, afterNum = l - beforeNum,
...
;(function ($) {
    ...
    var BaseControl = function () {
        this.options = {prefix: 'ms-', autohide: true, overVideo: true, customClass: null};
    };
...
;(function ($) {
    ...
    p.create = function () {
        _super.create.call(this);
        this.__translate_end = window._css3d ? ' translateZ(0px)' : '';
        this.controller = new Controller(0, 0, {snappingMinSpeed: 2, friction: (100 - this.options.speed * 0.5) / 100});
...
;(function ($) {
    ...
    p.create = function () {
        ...
        this.$element.click(function () {
            if (that.slider.api.paused) that.slider.api.resume(); else
...
;(function ($) {
    window.MSFacebookGallery = function (slider, options) {
        var _options = {count: 10, type: 'photostream', thumbSize: '320', imgSize: 'orginal', https: false};
        this.slider = slider;
        this.slider.holdOn();
        $.extend(_options, options);
        this.options = _options;
...
;(function ($, document, window) {
    ...
    var _scrollTo = function (target, duration) {
        ...
        $('html, body').animate({scrollTop: target.offset().top}, duration * 1000, 'easeInOutQuad');
    };
